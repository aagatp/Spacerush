...
namespace Collision
{
	...
	bool PixelPerfectTest(std::shared_ptr<Spaceship>& sp1, sf::Sprite& pickup, sf::Uint8 AlphaLimit) {
		sf::FloatRect Intersection;
		if (pickup.getGlobalBounds().intersects(sp1->getBounds(), Intersection)) {
			sf::IntRect O1SubRect = pickup.getTextureRect();
			sf::IntRect O2SubRect = sp1->getTextureRect();

			sf::Uint8* mask1 = Bitmasks.GetMask(pickup.getTexture());
			sf::Uint8* mask2 = Bitmasks.GetMask(sp1->getTexture());
			...
			for (int i = Intersection.left; i < Intersection.left + Intersection.width; i++) {
				for (int j = Intersection.top; j < Intersection.top + Intersection.height; j++) {
					...
					sf::Vector2f o1v = pickup.getInverseTransform().transformPoint(i, j);
					sf::Vector2f o2v = sp1->getInverseTransform().transformPoint(i, j);
					...
					if (o1v.x > 0 && o1v.y > 0 && o2v.x > 0 && o2v.y > 0 &&
						...
						if (Bitmasks.GetPixel(mask1, pickup.getTexture(), (int)(o1v.x) + O1SubRect.left, (int)(o1v.y) + O1SubRect.top) > AlphaLimit&&
							Bitmasks.GetPixel(mask2, sp1->getTexture(), (int)(o2v.x) + O2SubRect.left, (int)(o2v.y) + O2SubRect.top) > AlphaLimit)
						{
							return true;
						}
